...
export default function RealTimeNews() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      setFetchError(false);
      try {
        const apiUrl = `/api/news?limit=30`;
    ...
    if (!newsSocketRef.current || newsSocketRef.current.readyState === WebSocket.CLOSED) {
      const wsUrl = process.env.NEXT_PUBLIC_BACKEND_API_URL?.replace(/^http/, 'ws');
      const socket = new WebSocket(`${wsUrl}/ws/realtime-news`);
      ...
      socket.onmessage = (event) => {
        try {
          ...
          // Assuming the WebSocket sends individual news items
      ...
      >
        <AnimatePresence>
          {filteredNews.map((item) => {
              const sourceName = item.source ? (item.source.startsWith('http://') || item.source.startsWith('https://') ? new URL(item.source).hostname.replace('www.', '') : item.source) : '알 수 없음';
              const isClickable = item.link && (item.link.startsWith('http://') || item.link.startsWith('https://'));
              ...
              return (
                <Component
                  ...
                  layout
                  className={`block bg-slate-800/50 border border-slate-700/50 rounded-lg p-4 transition-all ${isClickable ? 'hover:bg-slate-800 hover:border-indigo-500/50 cursor-pointer' : 'cursor-default'}`}
                >
