...
  return (
    /* 수정: p-4 sm:p-6 로 변경하여 모바일에서 여백을 줄임 */
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
      <div className="flex items-center gap-2 mb-6">
        <Settings className="h-6 w-6" />
        {/* 수정: text-xl sm:text-2xl 로 변경하여 모바일에서 글자 크기 조절 */}
        <h1 className="text-xl sm:text-2xl font-bold">뉴스 스크래핑 설정</h1>
      </div>
...
                <div className="flex-1 mr-4"> {/* 수정: 오른쪽 버튼과의 간격을 위해 mr-4 추가 */}
                  <div className="flex items-center gap-3">
                    <Switch checked={source.enabled} onCheckedChange={() => toggleSource(source.id)} />
                    <div>
                      <h3 className="font-medium">{source.name}</h3>
                        {/* 수정: break-all 클래스를 추가해 긴 URL이 줄바꿈 되도록 함 */}
                      <p className="text-sm text-gray-500 break-all">{source.url}</p>
...
                <Button
                  variant="outline"
                  size="icon" /* 수정: size="sm" -> size="icon" 으로 변경하여 버튼 크기를 줄임 */
                  onClick={() => removeRSSSource(source.id)}
                  className="text-red-600 hover:text-red-700 shrink-0" /* shrink-0 추가로 버튼이 찌그러지지 않게 함 */
                >
